// Necessary includes for data handling, networking, and other utilities
#include <cstdlib>
#include <cstring>
#include <iomanip>
#include <vector>
#include <iterator>
#include <iostream>
#include <base64.h> // For base64 encoding and decoding
#include <boost/asio.hpp> // For network operations
#include <boost/uuid/uuid.hpp> // For handling unique identifiers
#include <boost/uuid/uuid_io.hpp> 
#include <vector>

#include "cryptlib.h"

// Predefined constants for the length of various fields in the protocol
#define NAME_LENGTH 255
#define RESPONSE_HEADER 7
...
/** PDU code  **/
// Predefined constants for the different types of request/response codes used in the protocol
#define REQUEST_CODE_1025 1025
#define REQUEST_CODE_1026 1026
#define REQUEST_CODE_1027 1027
#define REQUEST_CODE_1028 1028
#define REQUEST_CODE_1029 1029
#define REQUEST_CODE_1030 1030
#define REQUEST_CODE_1031 1031
#define REQUEST_CODE_END_SUCCESS 0
#define REQUEST_CODE_END_FAILURE 1

#define RESPONSE_CODE_2100 2100
#define RESPONSE_CODE_2101 2101
#define RESPONSE_CODE_2102 2102
#define RESPONSE_CODE_2103 2103
#define RESPONSE_CODE_2104 2104
#define RESPONSE_CODE_2105 2105
#define RESPONSE_CODE_2106 2106
#define RESPONSE_CODE_2107 2107
...
// 
// END CRC
// Using the tcp type from the boost::asio::ip namespace for network communications

using boost::asio::ip::tcp;

// Struct definition for the response header, which contains version, code, and payload size
...
// Struct definition for the request header, which contains client ID, version, code, and payload size
...
// Class definition for Protocol, which will handle encoding and decoding of data, as well as network communication protocols
class Protocol
{
	// Utility function to convert payload data to hexadecimal string for easy logging and debugging
...
public:
	...
	inline unsigned int crc32(char* data, size_t size)
	{
		...
		for (int i = 0; i < size; i++) {
			tabidx = (s >> 24) ^ (unsigned char)data[i];
		...
		return (unsigned int)UNSIGNED(~s);
	}

	// Decoding functions for various data types within the payload
	int decode_content_size(unsigned char* payload); // Decode content size from the payload
	std::string decode_file_name(unsigned char* payload); // Decode file name from the payload
	unsigned int decode_check_sum(unsigned char* payload); // Decode checksum from the payload

	// Encoding functions for various data types to be sent in the payload
	unsigned char* encode_file(char* name); // Encode file data for transmission
	std::string decode_crc(unsigned char*); // Decode CRC from the payload

	unsigned char* encode_name(std::string name); // Encode user's name for transmission
	std::string decode_name(unsigned char*); // Decode user's name from the payload

	unsigned char* encode_client_id(boost::uuids::uuid client_id); // Encode UUID for client identification
	boost::uuids::uuid decode_client_id(unsigned char* payload);  // Decode UUID from the payload

	void decode_client_id(std::string, uint8_t* out);	 // Overloaded function for UUID decoding
	std::string decode_client_id(unsigned char* payload, int y);	 // Overloaded function for UUID decoding with additional parameters

	// Functions related to encryption keys
	unsigned char* encode_public_key(std::string public_key);	 // Encode public key for secure communication
	char* decode_aes(unsigned char* payload, int len);	// Decode AES encrypted data from the payload

	// Functions for handling the headers of requests and responses
	request_header* encode_header(std::string client_id, int payload_size, int code);	 // Encode header data for requests
	response_header decode_header(tcp::socket& s);	// Decode header data from responses

	// Utility function to convert a buffer of 4 bytes to an integer, commonly used for size and length fields
	inline int calc_to_int(unsigned char buffer[], int pos) {
		unsigned int checksum = 0;

		checksum = ((unsigned char)buffer[pos]) << 24 |
			((unsigned char)buffer[pos + 1]) << ID_LENGTH |
